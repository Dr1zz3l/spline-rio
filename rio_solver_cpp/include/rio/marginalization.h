...
struct MarginalizationPrior {
    ...
    // Dimensions: d_b × d_b, d_b = 3*n_bound_pos + 3*n_bound_ori + 6 (+1 when
    // has_zbias: the radar z-bias state trails the bias block)
    ...
    // Radar z-bias state (cfg.radar_zbias_state): when has_zbias, the layout
    // gains one trailing coordinate after the bias block and d_b includes it.
    double radar_zbias{0.0};
    bool   has_zbias{false};
    ...
    // theta_swept: accumulated knot-to-knot rotation over the marginalized
    // zone (rad) -- the staleness regressor for deriving marg_stale_phi.
    double      theta_swept{0.0};
...
struct MargPriorFunctor {
    ...
    bool operator()(T const* const* params, T* residuals) const {
        ...
        // Radar z-bias state (trailing coordinate when present)
        if (prior.has_zbias) {
            const T* z = params[n_pos + n_ori + 1];
            local_x(offset++) = z[0] - T(prior.radar_zbias);
        }
