...
// ZbiasPriorFunctor
// ============================================================================
// Absolute prior on the radar z-bias state: r = (b_z - 0) / sigma_prior.
// Anchored at ZERO (the unbiased-sensor hypothesis), never the warm start —
// the same discipline as BiasPriorFunctor.  The random walk in the marg prior
// carries the window-to-window freedom.
struct ZbiasPriorFunctor {
    double inv_sigma;
    explicit ZbiasPriorFunctor(double sigma)
        : inv_sigma(sigma > 0.0 ? 1.0 / sigma : 0.0) {}

    template <class T>
    bool operator()(T const* const* params, T* residuals) const {
        residuals[0] = T(inv_sigma) * params[0][0];
        return true;
    }
};
