...
struct Trajectory {
    ...
    // Radar z-bias state b_z (m/s per unit BODY-frame u_z): slowly-varying
    // Doppler elevation bias, v_corr = v_meas - b_z * u_z(body).  Optimized
    // when cfg.radar_zbias_state (SW frame-whitened path only); random walk
    // across windows.  Measured basis: worklog/2026-08-12_zbias-state-measurement.md.
    ...
    double* radar_zbias_data() { return &radar_zbias; }
    const double* radar_zbias_data() const { return &radar_zbias; }
