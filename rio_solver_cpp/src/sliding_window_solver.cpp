    ...
    bool with_zbias = false)
{
    ...
    if (with_zbias) {
        eval_blocks.push_back(traj.radar_zbias_data());
        block_map.push_back({"radar_zbias", 0, col, 1});
        col += 1;
    }
}
    ...
    double t_start, double t_end, double stride)
{
    ...
    // Radar z-bias state: supported combinations only (see solver.h).
    const bool zb_on = cfg_.radar_zbias_state != 0;
    if (zb_on) {
        if (cfg_.lock_extrinsics == 0)
            throw std::runtime_error("radar_zbias_state requires lock_extrinsics=1");
        if (cfg_.radar_frame_sigma_c <= 0.0)
            throw std::runtime_error("radar_zbias_state requires the frame-whitened "
                                     "radar path (radar_frame_sigma_c > 0)");
        if (cfg_.radar_frame_sigma_z > 0.0)
            throw std::runtime_error("radar_zbias_state and radar_frame_sigma_z are "
                                     "competing instruments (the z-whitening direction "
                                     "IS the b_z Jacobian column); enable only one");
        if (cfg_.radar_zbias_fixed != 0.0)
            throw std::runtime_error("radar_zbias_state and radar_zbias_fixed must "
                                     "not be combined");
        if (cfg_.radar_pos_split > 0.0)
            throw std::runtime_error("radar_zbias_state does not support "
                                     "radar_pos_split");
    }
    if (cfg_.huber_model_knee && cfg_.huber_omega_sigma > 0.0)
        throw std::runtime_error("huber_model_knee and huber_omega_sigma are "
                                 "alternative knee forms; enable only one");
    ...
    if (zb_on)
        problem.AddParameterBlock(traj_.radar_zbias_data(), 1);
    ...
    if (prior_.valid) {
        ...
        prior_.radar_zbias = traj_.radar_zbias;
    ...
    for (const auto& frame : radar_frames) {
        ...
        double w_omega = 1.0;
        double huber_delta_eff = cfg_.huber_delta;
        ...
        if (cfg_.omega_gate_threshold > 0.0 || cfg_.omega_soft_sigma > 0.0
            ...
            // GP-style ω-scaled Huber knee: the GP applies Huber in WHITENED
            // units, so its raw-m/s inlier band widens as σ_eff(ω) grows
            // (~1.7 m/s mid-flip vs the fixed 1.0 here).  Opt-in port.
            if (cfg_.huber_omega_sigma > 0.0) {
                const double qh = w_norm / cfg_.huber_omega_sigma;
                huber_delta_eff = cfg_.huber_delta * std::sqrt(1.0 + qh * qh);
            }
        }
        ...
        // ---- Per-frame whitened factor (stochastic noise model) ------------
        // One n-dim residual block per frame; correlated component sigma_c
        // whitened exactly inside the factor.  Same parameter-block support
        // as the per-point factors (plus pitch_delta when extrinsics are
        // optimized), so marginalization bookkeeping is unchanged.
        if (cfg_.radar_frame_sigma_c > 0.0) {
            std::vector<std::unique_ptr<analytic::RadarAnalyticFactor>> pfs;
            std::vector<std::unique_ptr<analytic::RadarAnalyticWithPitchFactor>> pfs_p;
            std::vector<double> sws;
            std::vector<double> uzs;   // body-frame u_z per kept point
            for (const auto& pt : frame.points) {
                ...
                if (optimize_ext)
                    pfs_p.emplace_back(new analytic::RadarAnalyticWithPitchFactor(
                        u_sensor, v_meas_c, u_ori, inv_dt_ori, u_pos, inv_dt_pos,
                        R_radar_to_body, t_body_sensor));
                else
                    pfs.emplace_back(new analytic::RadarAnalyticFactor(
                        u_sensor, v_meas_c, u_ori, inv_dt_ori, u_pos, inv_dt_pos,
                        R_radar_to_body, t_body_sensor, zb_on));
                sws.push_back(std::sqrt(w_int * pt.w));
                uzs.push_back((R_radar_to_body * u_sensor).z());
            }
            const size_t n_pts = optimize_ext ? pfs_p.size() : pfs.size();
            if (n_pts == 0) continue;
            ...
            if (optimize_ext) params.push_back(traj_.pitch_delta_data());
            if (zb_on) params.push_back(traj_.radar_zbias_data());
            // Per-point robustness in frame mode: Huber IRLS weights
            // w = min(1, delta/|r|) from the WARM-START residuals, fixed at
            // problem build (same pattern as the omega soft gate).  Restores
            // the per-point outlier defense the shared frame block loses.
            if (cfg_.huber_delta > 0.0 && cfg_.huber_delta < 100.0) {
                for (size_t i = 0; i < n_pts; ++i) {
                    double r_ws = 0.0;
                    const bool ok = optimize_ext
                        ? pfs_p[i]->Evaluate(params.data(), &r_ws, nullptr)
                        : pfs[i]->Evaluate(params.data(), &r_ws, nullptr);
                    if (ok) {
                        // Model-sigma knee (huber_model_knee): knee at a fixed
                        // number of MODEL sigmas, delta_i = delta / sqrt(w_i);
                        // sws[i] here is exactly sqrt(w_int * pt.w) (pre-IRLS).
                        const double d_i = cfg_.huber_model_knee
                            ? cfg_.huber_delta / std::max(sws[i], 1e-3)
                            : huber_delta_eff;
                        const double a = std::abs(r_ws);
                        if (a > d_i)
                            sws[i] *= std::sqrt(d_i / a);
                    }
                }
            }
            ceres::CostFunction* fcost;
            if (optimize_ext)
                fcost = new analytic::RadarFrameWhitenedWithPitchFactor(
                    std::move(pfs_p), std::move(sws), cfg_.radar_frame_sigma_c,
                    cfg_.radar_frame_hetero != 0, uzs,
                    cfg_.radar_frame_sigma_z);
            else
                fcost = new analytic::RadarFrameWhitenedFactor(
                    std::move(pfs), std::move(sws), cfg_.radar_frame_sigma_c,
                    cfg_.radar_frame_hetero != 0, uzs,
                    cfg_.radar_frame_sigma_z, zb_on);
            ceres::LossFunction* floss = nullptr;   // robustness via IRLS weights
            const double s_frame = w_omega * cfg_.radar_weight;
            if (s_frame != 1.0)
                floss = new ceres::ScaledLoss(nullptr, s_frame, ceres::TAKE_OWNERSHIP);
            problem.AddResidualBlock(fcost, floss, params);
            continue;   // frame handled; next frame
        }
        ...
        for (const auto& pt : frame.points) {
            ...
            // Per-point intensity weight w = (I/I_med)^α (no clamp: measured
            // per-point I/I_med spans 0.37-3.3 own-platform, <=4.3 ICINS)
            double w_int = 1.0;
            if (inv_med_intensity > 0.0 && pt.intensity > 0.0) {
                w_int = std::pow(pt.intensity * inv_med_intensity,
                                 cfg_.radar_intensity_weight);
            ...
            const double s_pt = w_omega * w_int * cfg_.radar_weight * pt.w;
            if (s_pt != 1.0)
                loss = new ceres::ScaledLoss(
                    new ceres::HuberLoss(cfg_.huber_delta), s_pt,
    ...
    // ---- Radar z-bias state prior (anchored at zero) ------------------------
    if (zb_on && cfg_.radar_zbias_prior_sigma > 0.0) {
        auto* f = new ZbiasPriorFunctor(cfg_.radar_zbias_prior_sigma);
        auto* cost = make_auto_cost_sw(f, 1, {1});
        problem.AddResidualBlock(cost, nullptr, {traj_.radar_zbias_data()});
    }
    ...
    // With boundary_first_window_only the pins are added only while no
    // marginalization prior carries the past (2026-09-07 Fig. 1 audit: they
    // used to be re-added every window and marginalized into the next prior).
    if (!(cfg_.boundary_first_window_only && had_prior)) {
    ...
    if (cfg_.dump_system) {
        build_dump_blocks_window(traj_, oi0, oi1, pi0, pi1, optimize_ext,
                                 constant_blocks, dump_blocks, dump_bmap, zb_on);
    }
    ...
    if (cfg_.use_banded_schur) {
        ...
        // Globals = bias (6) + pitch_delta (1 if free ext) + z-bias state (1 if on)
        options.banded_n_global_cols = 6 + (optimize_ext ? 1 : 0) + (zb_on ? 1 : 0);
    } else {
    ...
    if (prior_.valid) {
        ...
        if (prior_.has_zbias) pparams.push_back(traj_.radar_zbias_data());
        ...
        // Staleness diagnostics: theta_swept was recorded when THIS prior was
        // built (previous window); dphi = mean update of the boundary ori
        // knots from their warm start (= prior_.bound_ori after the window-
        // start re-centering) to the solution -- the linearization movement
        // the prior should have budgeted.  k_derived = slope of dphi vs theta.
        result.marg_theta_swept = prior_.theta_swept;
        double dphi = 0.0;
        for (int i = 0; i < n_ori; ++i) {
            const auto& q0 = prior_.bound_ori[i];
            const auto& q1 = traj_.ori_knots[prior_.ori_start + i];
            const Sophus::SO3d Q0(Eigen::Quaterniond(q0[3], q0[0], q0[1], q0[2]));
            const Sophus::SO3d Q1(Eigen::Quaterniond(q1[3], q1[0], q1[1], q1[2]));
            dphi += (Q0.inverse() * Q1).log().norm();
        }
        result.marg_bound_dphi = n_ori > 0 ? dphi / n_ori : 0.0;
    }
    ...
    result.radar_zbias = traj_.radar_zbias;
...
void SlidingWindowSolver::add_prior_to_problem(ceres::Problem& problem) {
    ...
    if (prior_.has_zbias) cost->AddParameterBlock(1);
    ...
    params.push_back(traj_.bias_data());
    if (prior_.has_zbias) params.push_back(traj_.radar_zbias_data());
    ...
    bool had_prior)
{
    ...
    // d_b: local-param dimension of boundary + bias (+ z-bias state when on)
    const bool zb_on = cfg_.radar_zbias_state != 0;
    int d_b = 3 * n_bound_pos + 3 * n_bound_ori + 6 + (zb_on ? 1 : 0);
    ...
    if (zb_on) eval_blocks.push_back(traj_.radar_zbias_data());
    ...
    if (cfg_.marg_fast_prior) {
        ...
        std::unordered_map<const double*, ColInfo> colmap;
        {
            ...
            col += 6;
            if (zb_on) colmap[traj_.radar_zbias_data()] = {col, false};
        }
    ...
    // ---- Bias random walk across strides (stochastic noise model) -----------
    // Inflate the prior's bias covariance by Q via the information-space
    // Woodbury downdate S <- S - S P (Q^-1 + P^T S P)^-1 P^T S, where P
    // selects the bias coordinates (last 6 of the boundary layout).
    // Mean handling is trivial: the prior is re-centered every window.
    const double qz = zb_on ? cfg_.radar_zbias_rw_sigma : 0.0;
    // Rotation-swept staleness inflation (GP-backend port, opt-in): the
    // marginal prior's orientation rows were linearized at attitudes that a
    // fast rotation has since swept away from; inflate their covariance by
    // sigma_add = marg_stale_phi * theta_swept, theta_swept = accumulated
    // knot-to-knot rotation over the marginalized zone.  On the GP branch
    // this is the only mechanism whose ablation moves backflips position
    // (see worklog/2026-08-12 zbias battery entry).  Mean handling is
    // trivial here too: the prior is re-centered every window.
    double theta = 0.0;   // always computed: staleness regressor diagnostic
    for (int i = marg_ori_start; i <= marg_ori_end; ++i) {
        const auto& qa_ = traj_.ori_knots[i];
        const auto& qb_ = traj_.ori_knots[i + 1];
        const Sophus::SO3d Ra(Eigen::Quaterniond(qa_[3], qa_[0], qa_[1], qa_[2]));
        const Sophus::SO3d Rb(Eigen::Quaterniond(qb_[3], qb_[0], qb_[1], qb_[2]));
        theta += (Ra.inverse() * Rb).log().norm();
    }
    prior_.theta_swept = theta;
    double q_stale = 0.0;
    if (cfg_.marg_stale_phi > 0.0) {
        const double s_add = cfg_.marg_stale_phi * theta;
        q_stale = s_add * s_add;
    }
    double q_stale_pos = 0.0;   // position-row analogue, same regressor
    if (cfg_.marg_stale_pos > 0.0) {
        const double s_add = cfg_.marg_stale_pos * theta;
        q_stale_pos = s_add * s_add;
    }
    if (cfg_.bias_rw_sigma_accel > 0.0 || cfg_.bias_rw_sigma_gyro > 0.0
        || qz > 0.0 || q_stale > 0.0 || q_stale_pos > 0.0) {
        std::vector<int> idx;
        std::vector<double> qv;
        const double qa = cfg_.bias_rw_sigma_accel;
        const double qg = cfg_.bias_rw_sigma_gyro;
        // Bias coordinates sit at the end of the boundary layout, followed by
        // the z-bias state when present (d_b already includes it).
        const int off_bias = d_b - 6 - (zb_on ? 1 : 0);
        for (int i = 0; i < 3; ++i)
            if (qa > 0.0) { idx.push_back(off_bias + i); qv.push_back(qa * qa); }
        for (int i = 0; i < 3; ++i)
            if (qg > 0.0) { idx.push_back(off_bias + 3 + i); qv.push_back(qg * qg); }
        if (qz > 0.0) { idx.push_back(d_b - 1); qv.push_back(qz * qz); }
        if (q_stale > 0.0) {
            // Boundary-orientation tangent coordinates follow the pos block.
            const int off_ori = 3 * n_bound_pos;
            for (int i = 0; i < 3 * n_bound_ori; ++i) {
                idx.push_back(off_ori + i);
                qv.push_back(q_stale);
            }
        }
        if (q_stale_pos > 0.0) {
            // Boundary-position coordinates lead the layout.
            for (int i = 0; i < 3 * n_bound_pos; ++i) {
                idx.push_back(i);
                qv.push_back(q_stale_pos);
            }
        }
        const int k = static_cast<int>(idx.size());
        if (k > 0) {
            Eigen::MatrixXd SP(d_b, k);
            for (int j = 0; j < k; ++j) SP.col(j) = S.col(idx[j]);
            Eigen::MatrixXd M = Eigen::MatrixXd::Zero(k, k);
            for (int j = 0; j < k; ++j) {
                M(j, j) = 1.0 / qv[j];
                for (int l = 0; l < k; ++l) M(j, l) += S(idx[j], idx[l]);
            }
            Eigen::LDLT<Eigen::MatrixXd> ldltM(M);
            if (ldltM.info() == Eigen::Success) {
                S.noalias() -= SP * ldltM.solve(SP.transpose());
                S = 0.5 * (S + S.transpose());   // re-symmetrize
            }
        }
    }
