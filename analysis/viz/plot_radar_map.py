...
_R_base = rotation_matrix_from_euler(
    ...
    np.radians(_rotation_euler_deg[2]),
)
...
def build_extrinsics(flip: bool):
    if flip:
        return _R_yaw_flip @ _t_base, _R_yaw_flip @ _R_base
    else:
        return _t_base.copy(), _R_base.copy()
...
def main():
    ...
    # Timing offsets (from config/extrinsics.yaml)
