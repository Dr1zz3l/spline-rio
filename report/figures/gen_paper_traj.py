...
# Bags for which the batch estimate is omitted from the trajectory plot.
# Backflips batch is bistable at dt_o=0.8ms (Sec. VII-B); the SW live edge is
# the trustworthy result, so the figure shows MoCap + SW live only.
HIDE_BATCH = {'backflips_best_velocity'}
...
for bag_key, mocap_tag in BAGS:
    ...
    show_batch = bag_key not in HIDE_BATCH
