...
and $3.2$ (fast), within or just below the $\chi^2_3$ 95\% interval for the
window count (${\approx}[2.0,3.7]$): orientation covariance is calibrated on
fast racing and slightly conservative on slow, as the weights approximate
effective-noise whitening.  Where the finite-difference velocity reference is
clean (slow racing) the velocity \ac{nees} mean is $2.7$, also calibrated;
on the faster and tumbling flights that reference is itself too noisy
(${\sim}1$\,m/s) for a powered velocity check.  These are \emph{calibration
