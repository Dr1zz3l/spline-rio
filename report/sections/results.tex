...
Per-window solve times are 0.35--0.70\,s on a laptop CPU.  At the default
0.3\,s stride (at which all \autoref{tab:sw} accuracy is reported) the
system is therefore \emph{not} real-time.  Only fast racing reaches real
time, and only at a coarser 0.4\,s stride (its 0.35\,s solve fits that
budget); slow racing at 0.70\,s/window does not, at any practical stride.
