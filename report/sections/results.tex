...
\begin{table}[t]
\centering
\caption{Evaluation Configuration vs.\ Per-Regime Real-Time Operating Point}
\label{tab:realtime}
\small
...
\bottomrule
\multicolumn{5}{@{}p{\columnwidth}@{}}{\scriptsize Win./str.: window/stride.
  The 3\,s/0.3\,s evaluation configuration is used throughout the paper; the
  real-time point pushes per-window solve time below the stride.  Reaching real
  time is accuracy-neutral and improves backflips.  Live-edge \ac{rmse}; solve
  times carry $\sim$20\% run-to-run variance.}
\end{tabular}
\end{table}
