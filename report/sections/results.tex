...
On the racing flights both channels are calibrated to slightly conservative:
\emph{orientation} \ac{nees} mean $1.6$ (slow, $N{=}49$) and $3.2$ (fast,
$N{=}20$), \emph{velocity} $2.0$ and $1.0$, within or just below the
$N$-window 95\% interval (slow $[2.4,3.7]$, fast $[2.0,4.2]$) --- the
weights approximate effective-noise whitening, mildly over-reporting
velocity uncertainty.  These are \emph{calibration checks}, not powered
hypothesis tests: consecutive windows are temporally correlated, so the
expectation holds only heuristically.  On backflips only 6 of 50 windows
