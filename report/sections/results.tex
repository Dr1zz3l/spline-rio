...
fast is real-time at the 0.3\,s stride with a 2\,s
...
\centering
\footnotesize
% (a) reports drift [\% path] = ATE / trajectory length: normalized across the
% four different path lengths, easier to read than meters, and the exact metric
% (b) decomposes into horizontal/vertical.  Absolute ATE [m] is preserved in
% (b)'s whole-traj column for the published cross-check.  Both subtables fill
% \textwidth (via \extracolsep{\fill}) so the float leaves no left/right
% whitespace and they share one axis.
\setlength{\tabcolsep}{6pt}%
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccc}
\toprule
\multicolumn{4}{l}{\textit{(a)~Headline: drift [\% path] / velocity [m/s] / orientation [\degree]}}\\
\midrule
...
\parbox{\textwidth}{\raggedright\footnotesize Prefilter \emph{disabled} (Huber-only): our vertical drift $12$--$15\%$; whole-traj.\ \ac{ate} $9.57/2.88/10.86/5.48$\,m (flights~1--4).\par}
\end{table*}
