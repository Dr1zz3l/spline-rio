...
This same-order position is what the default \ac{ransac} prefilter
...
a design-space split (\autoref{sec:conclusion}), not a defect of either
method.
...
$0.7$--$0.9{\times}$ its consistent value (essentially calibrated, as the
measurement weights approximate effective-noise whitening), and the
velocity \ac{nees} is mildly overconfident (median $1.4$--$5.7$; the
heavier-tailed mean is inflated by a few windows where the
finite-difference \ac{mocap} velocity reference is itself unreliable,
so we take the median).  Both are \emph{calibration checks}, not powered
