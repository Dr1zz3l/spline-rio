...
We use a single 3\,s window and 0.3\,s stride as the \emph{working point}
throughout this paper, for consistency across tables and plots rather than
because it is real-time: per-window solve time is set by the window length
and knot density, not the stride, and at 3\,s only fast racing approaches
the 0.3\,s budget on a laptop CPU.  As a check, pushing the window and
stride to \emph{barely} reach real time per regime costs almost nothing
(\autoref{tab:realtime}, \autoref{fig:realtime}): fast is real-time at the
0.3\,s stride with a 2\,s window, and slow and backflips at a 0.5\,s stride
(1.5\,s and 2\,s windows), with live accuracy within run-to-run noise of
the working point.  Backflips even improves
(1.55$\to$1.50\,m, 6.35$\to$5.56\degree), and racing stays under 1\% drift.
Solve times carry $\sim$20\% run-to-run variance, so the strides are taken
with margin.  For comfortable onboard margins the lever is the linear
solve: a banded Schur factorization is numerically unstable at our
conditioning, leaving incremental smoothing, not batch re-solve, the route.
Girod et al.~\cite{girod2024brio} reach onboard real time with exactly such
an iSAM2 back-end on a discrete-time graph; combining incremental inference
with our continuous-time B-spline, where the shared global bias induces a
Bayes-tree fill-in, is the natural next step we leave to future work.
...
\caption{\textcolor{red}{[Choose this table \emph{or}
  \autoref{fig:realtime}.] Unified working point (3\,s/0.3\,s, used
...
\midrule
\multirow{2}{*}{Fast}
  & 3.0/0.3 (working)   & 0.32 & 0.399 & 2.88 \\
  & 2.0/0.3 (real-time) & 0.28 & 0.443 & 2.95 \\
\midrule
\multirow{2}{*}{Slow}
  & 3.0/0.3 (working)   & 0.72 & 0.310 & 1.97 \\
  & 1.5/0.5 (real-time) & 0.40 & 0.301 & 1.98 \\
\midrule
\multirow{2}{*}{Backflips}
  & 3.0/0.3 (working)   & 0.59 & 1.545 & 6.35 \\
