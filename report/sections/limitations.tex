...
the heading-aided orientation numbers are an \emph{upper bound} on accuracy.
We characterize the gap with a perturbed heading prior (a slowly-varying
$2\degree$ drift, a low-frequency sinusoid with random phase/frequency, zero
at the start, plus $1.5\degree$ white noise; three seeds).  The estimate
\emph{tracks} the prior, so the effect is regime- and realization-dependent:
live orientation ranges $2.1$--$3.5\degree$ on slow racing (from
$1.97\degree$: a sustained drift propagates into yaw at $\lambda_\psi{=}10$),
$2.9$--$3.3\degree$ on fast (from $2.88\degree$, weaker prior), and
$6.4$--$6.9\degree$ on backflips (from $6.35\degree$, roll/pitch-dominated).
Real magnetometers can be larger and throttle-correlated, so this is an
optimistic floor; real-mag validation is future work (\autoref{sec:models}).
