...
| `report/main.tex` | **T-RO journal submission — the single maintained paper.** Methodology, ablations, negative results. **`paper/` (conference cut) is FROZEN (2026-06-24): do NOT edit it.** Only `report/` is updated; regenerate a conference cut from `report/` if ever needed. |
