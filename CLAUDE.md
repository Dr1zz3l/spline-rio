...
| `report/main.tex` | **T-RO journal submission — the single maintained paper.** Thin root: preamble + `\input{acronyms}` + `\input{sections/*.tex}` (one file per section) + bib/bios. Figures in `report/figures/`; acronyms in `report/acronyms.tex`. **`paper/` (conference cut) is FROZEN (2026-06-24): do NOT edit it.** Regenerate a conference cut from `report/` if ever needed. |
