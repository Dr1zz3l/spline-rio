...
| `report/main.tex` | **T-RO journal submission — the single maintained paper. Trimmed to the hard 12pp limit (2026-06-25).** Thin root: preamble + `\input{acronyms}` + `\input{sections/*.tex}` (one file per section) + bib/bios. Figures in `report/figures/`; acronyms in `report/acronyms.tex`. **Figure set (post-trim):** Fig 1 pipeline (TikZ inline, scalebox 0.78), Fig 2 SW-marginalization (TikZ inline, scalebox 0.82), Fig 3 traj overlays (`gen_combined_traj.py`, figsize h=2.45), Fig 4 error-over-time (`gen_error_time.py`, 1.95/row). **Removed for the page budget:** the RPE figure (`fig:rpe`/`gen_rpe.py` — data now repo-only, one-line pointer in Experimental Setup) and the prior-scale plot (`fig:prior_scale`/`gen_prior_scale.py` — redundant with Table VII `tab:prior_scale`, which keeps all numbers). **Table V (`tab:baselines`) is single-column** ((a) drift%/vel/ori `\footnotesize`, (b) decomposition `\scriptsize`); 23 refs (dropped Hug et al.). **`paper/` (conference cut) is FROZEN (2026-06-24): do NOT edit it.** Regenerate a conference cut from `report/` if ever needed. |
...
| `documentation/RESEARCH_NOTES.md` | Design rationale: solver perf profile, Doppler unwrapping, preintegration investigation, BandedSchurSolver post-mortem, SW timing analysis, real-time/speedup path (§1/§7-§10), iSAM2 speedup investigation (§11: adaptive extra_iters win + comp-filter/Cholesky negative, no substantial 2-10x without a new formulation); absorbed the deleted `Realtime_Options.md` |
