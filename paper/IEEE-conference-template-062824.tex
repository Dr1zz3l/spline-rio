...
``STEAM'' trajectories, and basalt VIO~\cite{usenko2020basalt} uses
cumulative B-splines on Lie groups for stereo-inertial odometry.  We
...
Ctrl-VIO~\cite{lv2022ctrlvio}, CLIC~\cite{lv2023clic}, and
...
Trommer~\cite{doer2020ekf} and extensions to multiple
radars~\cite{huang2024multiradar} and 4D-radar-aided inertial
...
unchanged.  It runs once at frame load, not per window, so it does not
...
Fig.~\ref{fig:traj} shows trajectory overlays for both bags.
...
\begin{figure}[t]
  ...
  \caption{Trajectory overlays (X--Y and Y--Z projections,
    start~$\blacksquare$): slow racing (top) and fast racing (bottom).
    MoCap ground truth (solid blue), batch estimate (dashed red), SW
    live edge (dotted orange).  Larger fast-racing deviations reflect
    higher dynamics ($\omega_{\max}=9.1$\,rad/s).}
  \label{fig:traj}
\end{figure}
