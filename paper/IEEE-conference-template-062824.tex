...
\begin{figure*}[t]
  ...
  \caption{Trajectory overlays for all three flights (columns), X--Y
    ...
    omitted (bistable, Sec.~\ref{sec:backflips}); its residual
    ${\sim}6\degree$ live orientation error attenuates the loop circles
    while the donut sweep and altitude are followed.}
  \label{fig:traj}
\end{figure*}
...
\textbf{(b)}~Decomposition: \emph{roll/pitch} (heading-independent, yaw
excluded) is comparable across systems, so the orientation gap in (a) is
yaw, not attitude; \emph{our drift} splits position into
horizontal/vertical (both sub-1.3\% under the default RANSAC prefilter);
\emph{whole-traj.\ ATE} uses EVO/KITTI Umeyama alignment for cross-checking
