...
\SI{0.46}{\metre\per\second}/\SI{1.9}{\degree}
on gentle racing and \SI{0.32}{\metre\per\second}/\SI{2.8}{\degree} on
aggressive racing at 0.35--0.70\,s per window on a laptop; position
drifts \SI{0.6}{\percent} and \SI{0.9}{\percent} of distance.
Backflips --- attitude-tumbling flight that exposes radar systematics
invisible in level flight --- reach
\SI{2.9}{\percent}\,/\,\SI{6.3}{\degree} live.
...
improves aggressive-racing live position by $22\%$ --- and it is enabled
by default for every result in this paper.
...
($0.30\degree$) and position are unchanged.
...
(Table~\ref{tab:baselines-decomp}) and bringing position to baseline
...
alignment (Table~\ref{tab:baselines-decomp}) all three systems agree to
...
\multicolumn{3}{@{}p{\columnwidth}@{}}{\scriptsize
  Absolute figures predate the default RANSAC front-end
  (Sec.~\ref{sec:models}), which shifts the slow\_racing batch default to
  0.198\,m/1.01\degree\ and leaves every ordering here intact.
...
the benefit is captured in the prior-scale sweep
(Sec.~\ref{sec:prior_scale}) instead.
...
(Table~\ref{tab:sw}).  The default RANSAC prefilter (Sec.~\ref{sec:models})
...
($2{\times}10^{-4}$: 0.825\,m) for inter-window position continuity ---
without it (scale\,$10^{-7}$) fast-racing position degrades to
