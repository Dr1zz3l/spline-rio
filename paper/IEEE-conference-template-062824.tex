...
truth evaluation.  The radar is mounted upside-down with a $30\degree$ downward tilt by
CAD design; because the mount is 3D-printed and hand-assembled, the
as-built angle is a few degrees off and is recovered by the batch
extrinsic self-calibration (Sec.~\ref{sec:results}), then frozen for the
sliding window.  The sensor-to-body extrinsic rotation is parameterised
as $Z$--$Y$--$X$ Euler angles $[\text{roll},\text{pitch},\text{yaw}] =
[180\degree, 25.5\degree, 0\degree]$, translation
...
downtilt; the $25.5\degree$ is a legacy value from an early calibration
that seeds the solve, not a measurement.  The radar operates at $\sim$11\,Hz with 0.049\,m/s Doppler
...
full-trajectory batch solve with the pitch left free: from the legacy
25.5\degree\ initialization it converges to 27.0\degree\ (slow racing)
and 27.2\degree\ (fast), a few degrees below the 30\degree\ CAD design
(3D-printed, hand-assembled fixture).  The pitch is only weakly
observable, however, so we freeze it at 27.5\degree\ for all
sliding-window runs rather than estimate it --- the self-calibration is
init-dependent and the RMSE forms a plateau (Sec.~\ref{sec:backflips}).
...
initialization ($25.5\degree{\to}27\degree$, $30\degree{\to}36\degree$
on both racing bags, diverging from a saddle near $23\degree$).  The
earlier ``self-calibrated $+2\degree$ absorbs the elevation bias''
reading is retired; $27.5\degree$ is a value within this plateau,
consistent with the hand-built mount sitting a few degrees below the
$30\degree$ CAD design (the 3D-printed, loosely-fastened fixture
