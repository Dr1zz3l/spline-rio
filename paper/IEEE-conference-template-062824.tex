...
degrade in dusty, foggy or featureless environments.
...
document the prior-scaling challenges when the marginal's orientation
and position information differ by ten orders of magnitude.
...
Absolute RMSE is not comparable across papers --- our own quantization
...
numbers we cross-validate against this line directly: Sec.~\ref{sec:baselines} runs our system on the
...
truth evaluation.  The radar is mounted upside-down at a nominal $30\degree$ downtilt (CAD);
the as-built angle is a few degrees lower (Sec.~\ref{sec:results}), and
the sensor-to-body extrinsic rotation is $Z$--$Y$--$X$ Euler angles
$[\text{roll},\text{pitch},\text{yaw}] = [180\degree, 27.5\degree,
0\degree]$, translation $[0.08, 0.02, -0.01]$\,m.  The radar boresight
lies along the body forward axis, so the Euler pitch is numerically the
physical boresight downtilt.  The radar operates at $\sim$11\,Hz with 0.049\,m/s Doppler
resolution (best-velocity firmware configuration).  Raw IMU rate
is $\sim$993\,Hz; both modalities are used at full rate.

\subsection{Pipeline}
Because the B-spline optimization is nonlinear, it needs an initial
trajectory to converge; a three-stage sensor-only pipeline constructs
one from sensor data alone, which seeds a C++ Ceres solver (batch or
sliding window) --- the raw measurements then also enter the solver
directly as factors:
...
\text{pitch}{=}27.5\degree,\,\text{yaw}{=}0\degree]$), and
...
The radar-to-body pitch is a fixed mount angle, nominally 30\degree\ by
CAD but lower as-built (3D-printed, hand-assembled fixture).  Two
independent estimates agree on ${\approx}27.5\degree$: an inclinometer
on the radar face reads 27--28\degree\ (reference surface 0\degree), and
the batch self-calibration on the flight data converges to
27.0\degree/27.2\degree\ (slow/fast).  We freeze pitch at 27.5\degree\
for all sliding-window runs rather than estimate it online --- it is
only weakly observable from Doppler (the self-calibration is
init-dependent, and orientation RMSE is pitch-insensitive;
Sec.~\ref{sec:backflips}), so the physical measurement anchors
it.\footnote{The flights span two campaigns with mechanically identical
radar mounts (the later adds crash protection); the inclinometer
measures the earlier mount and the self-calibration the later flights,
and both agree.}
...
Sec.~\ref{sec:results}), a few degrees below the $30\degree$ CAD design
