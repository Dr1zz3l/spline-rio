...
2. **Investigate z-velocity bias** — persistent -0.5 to -0.65 m/s error. Not caused by pitch angle error (2° → only 0.05 m/s). *(Correction 2026-08-12: an earlier version attributed this to “poor elevation diversity, only 2 TX antennas” — wrong for the AOP package, whose 3TX×4RX array has ~symmetric az/el resolution. Mechanism remains open; floor-multipath / scene anisotropy is the leading candidate, and the ICINS platform — same IWR6843AOP sensor — shows a larger same-signed bias on indoor scenes.)*
